...
# Where to find data
tf.app.flags.DEFINE_string('data_path', 'datasets/MexicoQuake_test.bin', 'Path expression to tf.Example datafiles. Can include wildcards to access multiple datafiles.')
tf.app.flags.DEFINE_string('vocab_path', 'datasets/vocab', 'Path expression to text vocabulary file.')
...
tf.app.flags.DEFINE_string('log_root', 'check', 'Root directory for all logging.')
...
tf.app.flags.DEFINE_integer('max_dec_steps', 200, 'max timesteps of decoder (max summary tokens)')
tf.app.flags.DEFINE_integer('beam_size', 5, 'beam size for beam search decoding.')
tf.app.flags.DEFINE_integer('min_dec_steps', 150, 'Minimum sequence length of generated summary. Applies only for beam search decoding mode')
...
tf.app.flags.DEFINE_integer('max_train_iter', 120000, 'max iterations to train')
...
def run_training(model, batcher, sess_context_manager, sv, summary_writer):
  ...
  with sess_context_manager as sess:
    ...
    coun=0
    for _ in range(FLAGS.max_train_iter): # repeats until interrupted                      while True: # repeats until interrupted
