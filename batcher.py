...
class Example(object):
  ...
  def __init__(self, article, abstract_sentences, keyphrase, vocab, hps):
    ...
    if hps.pointer_gen.value:
      ...
      #print self.enc_input_extend_vocab
      #print '----------------------------'
    ...
    # Project document to topic space
    #article_topic_words, self.article_topic_word_probs  = vocab.tm.get_doc_topics_words_probs(' '.join(article_words))#abstract_words
    article_topic_words, self.article_topic_word_probs  = vocab.tm.get_doc_topics_words_probs(' '.join(article_words), keyphrase, hps.max_enc_steps.value)#abstract_words not keyphrases is now input of this function
    self.article_topic_vector, self.article_topic_oovs = data.article_topicwords_2_ids(article_topic_words, vocab)
    #print len(self.article_topic_word_probs), len(self.article_topic_vector)
    self.article_topic_vector = self.article_topic_vector[:hps.max_enc_steps.value]# Truncate topic vector
    self.article_topic_word_probs = self.article_topic_word_probs[:hps.max_enc_steps.value]# Truncate topic vector
    #self.enc_topicwords_len = len(self.article_topic_vector)
    """The above Should be padded, this is done in the Batch class"""
    #You could probably remove the above trucation part for topic words as now the truncated version of the article is used as the input to topic computation.
    
    #print self.article_topic_vector
    #print self.article_topic_oovs
  ...
  def pad_encoder_topicvector_input(self, max_len, pad_id):
    """Pad the encoder input sequence with pad_id up to max_len."""
    while len(self.article_topic_vector) < max_len:
      self.article_topic_vector.append(pad_id)
      self.article_topic_word_probs.append(pad_id)
      #print "yayayyayayyaya"
...
class Batch(object):
  ...
  def init_encoder_seq(self, example_list, hps):
    ...
    # max_enc_seq_len is currently set to 40

    # Pad the encoder input sequences up to the length of the longest sequence
    for ex in example_list:
      ex.pad_encoder_input(max_enc_seq_len, self.pad_id)
      ex.pad_encoder_topicvector_input(max_enc_seq_len, self.pad_id)#padded the topic vector and topic word probs
    ...
    #enc_topicwords_batch contains all probabilities per word derived from the topic model
    self.enc_topicwords_batch = np.zeros((hps.batch_size.value, max_enc_seq_len), dtype=np.int32)
    self.enc_topicwords_probs_batch = np.zeros((hps.batch_size.value, max_enc_seq_len), dtype=np.int32)
...
class Batcher(object):
  ...
  def fill_example_queue(self):
    ...
    while True:
      try:
        (article, abstract, keyphrase) = next(input_gen) # read the next example from file. article and abstract are both strings.
      ...
      example = Example(article, abstract_sentences, keyphrase, self._vocab, self._hps) # Process into an Example.
  ...
  def text_generator(self, example_generator):
    ...
    while True:
      ...
      try:
        ...
        keyphrase_text = e.features.feature['keyphrase'].bytes_list.value[0].decode() # the abstract text was saved under the key 'abstract' in the data files
      except ValueError:
        tf.logging.error('Failed to get article or abstract or keyphrase from example')
        continue
      if len(article_text)==0: # See https://github.com/abisee/pointer-generator/issues/1
        tf.logging.warning('Found an example with empty article text. Skipping it.')
      else:
        yield (article_text, abstract_text, keyphrase_text)
