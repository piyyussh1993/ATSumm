...
class TopicModel (object):
    ...
    def __init__(self, modelAdd = 'datasets/LDA/lda.model', dictAdd = 'datasets/LDA/dictionary.dic', topicsFileAdd = ''):
    ...
    def _turnTopicOff (self, topicDict):
        ...
        return topicDict
    '''
    def _createTopicsDict (self, keyphrase, inp_len): # put all topics in a dictionary. A topic could be accessed by its number. Function returns a dictionary contating all topics.
        ...
        for i in range(len(x)-1):
            xx=x[i].split(" #")
            count=count+len(xx[0].split(' '))
            if count<=inp_len:
                kp.append(xx[0])
                kp_score.append(float(xx[1]))
        #kp, kp_score = DRAKE(doc, self.topicModel)
        tokens = [[token for token in sentence.split()] for sentence in kp]  #tokenisation
        t=[]
        for j in range(len(tokens)):
            token_dect={}
            for i in tokens[j]:
                if i in token_dect.keys():
                    token_dect[i] = 1 + token_dect[i]
                else:
                    token_dect[i] = 1
            t.append(token_dect)
        su=0
        ...
        for i in range(len(t)):
            for j in t[i]:
                if len(tokens[i])!=0:
                  t[i][j]=(t[i][j]/len(tokens[i]))*kp_score[i]
        KeyPhrase_dictionary ={}
        for j in range(len(t)):
            for i in t[j]:
                if i in KeyPhrase_dictionary.keys():
                    KeyPhrase_dictionary[i] = t[j][i] + KeyPhrase_dictionary[i]
                else:
                    KeyPhrase_dictionary[i] = t[j][i]
    ...
    def _doc2FinalWordVector (self, doc, topicProportions = True):
        ...
        return docTopics
    '''
    def get_doc_topics_words_probs (self, doc, keyphrase, inp_len):
        #bow = self.topicModelDictionary.doc2bow(doc.split())
        #docTopics = self.topicModel.get_document_topics(bow, per_word_topics=True)
        #docTopics = self._doc2FinalWordVector(doc, topicProportions=True)
        docKeyPhrasewords_with_prob = self._createTopicsDict(keyphrase, inp_len)
        words =[]
        prob_scores = []
        ...
        for w in doc_words:
            words.append(w)
            if w in docKeyPhrasewords_with_prob:
                prob_scores.append(docKeyPhrasewords_with_prob[w])
            else:
