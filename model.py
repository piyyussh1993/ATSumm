...
class SummarizationModel(object):
  ...
  def _add_placeholders(self):
    ...
    # topic words part
    self._enc_topicwords_batch = tf.placeholder (tf.int32, [hps.batch_size.value, None], name='enc_topicwords_batch')
    self._enc_topicwords_probs_batch = tf.placeholder (tf.float32, [hps.batch_size.value, None], name='enc_topicwords_probs_batch')
  ...
  def _make_feed_dict(self, batch, just_enc=False):
    ...
    # topic words part
    feed_dict[self._enc_topicwords_batch] = batch.enc_topicwords_batch
    feed_dict[self._enc_topicwords_probs_batch] = batch.enc_topicwords_probs_batch
  ...
  def _add_decoder(self, inputs):
    ...
    outputs, out_state, attn_dists, p_gens, coverage = attention_decoder(inputs, self._dec_in_state, self._enc_states, self._enc_topicwords_probs_batch, self._enc_padding_mask, cell, initial_state_attention=(hps.mode=="decode"), pointer_gen=hps.pointer_gen.value, use_coverage=hps.coverage.value, prev_coverage=prev_coverage)

    return outputs, out_state, attn_dists, p_gens, coverage

  def _calc_final_topicadded_dist(self, vocab_dists, attn_dists, topicwords_probs):
    ...
    with tf.variable_scope('final_distribution'):
        ...
        #final_dists = attn_by_topicprobs+final_dists
        return final_dists
  ...
  def _add_seq2seq(self):
    ...
    with tf.variable_scope('seq2seq'):
      ...
      if FLAGS.pointer_gen:
        ...
        """This modification gets creates a multiplication between the decoder outputs and the topicwords_probs as well as attention"""
        final_dists = self._calc_final_topicadded_dist(vocab_dists, self.attn_dists, self._enc_topicwords_probs_batch)
  ...
  def decode_onestep(self, sess, batch, latest_tokens, enc_states, dec_init_states, prev_coverage):
    ...
    # topic words part
    feed[self._enc_topicwords_batch] = batch.enc_topicwords_batch
    feed[self._enc_topicwords_probs_batch] = batch.enc_topicwords_probs_batch
